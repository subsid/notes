...
ex)
Let us split a line to some over all its points.
...
This is a clever of finding the probability of an event. Divide and conquer approach.
...
P(B) = \Sigma_(i=1 to n)P(B/A_i)P(A_i)
...
R.V X: number of heads that are observed.
Model of: Number of successes in a given number of independent trials.

PMF: p_X(k) = (^n_k)p^k(1-p)^(n-k)

*** Geometric R.V
Parameter: 0 < p <= 1
Experiment: Infinitely many independent tosses of a coin P(Heads) = p.
*note*: When we say infinitely many independent tosses, we mean that any finite subset of the tosses are independent.
Sample Space: Set of Infinite sequences of H and T.
Random Variable X: Number of tosses until first heads.
Model of: Waiting times, number of trials until a success.

PMF: P_X(k) = P(we get first heads in kth trial) = (1-p)^(1-k).p
